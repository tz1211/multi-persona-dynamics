...
# Usage: bash scripts/eval_checkpoints.sh [GPU_ID] [TRAIT] [LAYER]
...
# Log file
LOG_FILE="$RESULTS_DIR/evaluation_log_$(date +%Y%m%d_%H%M%S).txt"
...
echo "N per question: $N_PER_QUESTION" | tee -a "$LOG_FILE"
...
for CHECKPOINT in $CHECKPOINTS; do
    ...
    if CUDA_VISIBLE_DEVICES=$GPU python -m eval.eval_persona \
        ...
        --version "$VERSION" \
        --n_per_question "$N_PER_QUESTION" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    SHIFT_OUTPUT="$RESULTS_DIR/${CHECKPOINT_NAME}_shift.json"
    START_TIME=$(date +%s)
    ...
    if CUDA_VISIBLE_DEVICES=$GPU python -m eval.calc_finetuning_shift \
        ...
        --layer "$LAYER" \
        --output "$SHIFT_OUTPUT" 2>&1 | tee -a "$LOG_FILE"; then

        SHIFT_TIME=$(($(date +%s) - START_TIME))
        echo "  ✓ Finetuning shift completed in ${SHIFT_TIME}s" | tee -a "$LOG_FILE"
    else
        echo "  ✗ Finetuning shift failed for $CHECKPOINT_NAME" | tee -a "$LOG_FILE"
    fi
